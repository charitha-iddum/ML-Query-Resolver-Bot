...
def get_best_response(user_input, patterns, responses, bert_embeddings_patterns, tokenizer, model):
    preprocessed_input = preprocess_sentence(user_input)
    user_embedding = generate_bert_embeddings([preprocessed_input], tokenizer, model)
    similarity_scores = cosine_similarity(user_embedding, bert_embeddings_patterns)
